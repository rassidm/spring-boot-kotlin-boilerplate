...
class AuthIntegrationControllerTests : BaseIntegrationController() {
	...
	init {
		...
		Given("POST /api/v1/auth/signIn") {
			...
			val signInRequest: SignInRequest = mockSignInRequest.copy(email = defaultUserEmail, password = defaultUserPassword)
			...
			When("Field Valid Exception POST /api/v1/auth/signIn") {
				val wrongSignInRequest: SignInRequest = signInRequest.copy(email = "wrong_email_format", password = "1234")
			...
			When("UnAuthorized Exception POST /api/v1/auth/signIn") {
				val userUnAuthorizedException = UserUnAuthorizedException(user.id)

				every { authService.signIn(any<SignInCommand>()) } throws userUnAuthorizedException
			...
			When("Not Found Exception POST /api/v1/auth/signIn") {
				...
				every { authService.signIn(any<SignInCommand>()) } throws userNotFoundException
		...
		Given("POST /api/v1/auth/refresh") {
			...
			When("Not Found Exception POST /api/v1/auth/refresh") {
				...
				every { authService.refreshAccessToken(any<RefreshAccessTokenCommand>()) } throws refreshTokenNotFoundException
